  ...
  COLOR col)
{
  ...
  _pDP->SetTextScaling(_fResolutionScaling * _fCustomScaling * 0.25f);
  _pDP->PutText(strText, pixI, pixJ, col);
}

static void HUD_DrawBar2(FLOAT fX, FLOAT fY, FLOAT fSizeX, FLOAT fSizeY,
  enum BarOrientations eBarOrientation, FLOAT fNormValue, COLOR colDefault)
{
  COLOR col = colDefault;
...
extern void DrawHUD( const CPlayer *penPlayerCurrent, CDrawPort *pdpCurrent, BOOL bSnooping, const CPlayer *penPlayerOwner)
{
  ...
  PIX pixBorderSize = 1;
  FLOAT fMaxHealthBarSizeI = TOP_HEALTH * 0.7f;

  if (fValue > TOP_HEALTH)
  {
    fMaxHealthBarSizeI = fMaxHealth * 0.7f;
  }

  FLOAT fHealthBarSizeI = fValue * 0.7f;
  ...
  HUD_DrawBar2(fCol + pixBorderSize, fRow + pixBorderSize, fHealthBarSizeI, fHealthBarSizeJ, BO_LEFT, fNormValue, colHeathBar);
  ...
  FLOAT fMaxArmorBarSizeI = TOP_ARMOR * 0.7f;

  if (fValue > TOP_ARMOR)
  {
    fMaxArmorBarSizeI = fMaxArmor * 0.7f;
  }

  FLOAT fArmorBarSizeI = fValue * 0.7f;
  FLOAT fArmorBarSizeJ = 10.0f;
  COLOR colArmorBar = 0xadd8e600 | _ulAlphaHUD;

  fRow = pixTopBound + fHealthBarSizeJ + pixBorderSize * 2 + fLeftPadding;
  ...
  COLOR colText = 0x00FF0000 | _ulAlphaHUD;
  fCol = pixLeftBound;
  fRow = pixTopBound + fIconSizeJ + fTopPadding;
  ...
  if (ptoCurrentAmmo != NULL && !GetSP()->sp_bInfiniteAmmo) {
    ...
    HUD_DrawText2(fCol, fRow, strValue, colText);
  }
