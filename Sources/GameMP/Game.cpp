...
static CTextureObject _toBcgPopup;
...
void CGame::LCDInit(void)
{
  try {
    ...
    _toBcgPopup.SetData_t(CTFILENAME("TexturesMP\\General\\PopupBack.tex"));
    ...
    ((CTextureData*)_toBcgPopup .GetData())->Force(TEX_CONSTANT);
...
{
  if (_bPopup) {
    _pdp_SE->PutTexture(&_toBcgPopup, _boxScreen_SE, C_WHITE | 255);
  } else {
