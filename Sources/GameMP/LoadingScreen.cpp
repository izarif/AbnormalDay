...
static CTextureObject toLoading;

BOOL LoadLoadingTexture(void)
{
  try {
    toLoading.SetData_t(CTFILENAME("Textures\\General\\Loading.tex"));
    ((CTextureData*)toLoading.GetData())->Force(TEX_CONSTANT);
  }
  catch (char *strError) {
    CPrintF("%s\n", strError);
...
void FreeLoadingTexture(void)
{
  toLoading.SetData(NULL);
}
